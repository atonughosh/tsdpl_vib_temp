...
async def temperature_task():
    ...
    while True:
        try:
            ...
            data = (
                f"AccX: {ax:.8f}, " if ax is not None else "AccX: 0.0, "
            ) + (
                f"AccY: {ay:.8f}, " if ay is not None else "AccY: 0.0, "
            ) + (
                f"AccZ: {az:.8f}, " if az is not None else "AccZ: 0.0, "
            ) + f"Temp: {temperature:.4f}C, FW: {firmware_version}"
